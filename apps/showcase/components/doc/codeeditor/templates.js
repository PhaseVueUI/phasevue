...
const PhaseVue = {
    version: '^4.0.0',
    description:
        'PhaseVue is an open source UI library for Vue featuring a rich set of 80+ components, powerful theme customization, various theme alternatives such as Material, Bootstrap, Tailwind, premium templates and professional support. In addition, it integrates with PrimeBlock, which has 500+ ready to use UI blocks to build spectacular applications in no time.'
};
...
const core_dependencies = {
    ...
    vite: '^4.0.0',
    phasevue: pkg.version || PhaseVue.version || 'latest',
    '@phasevueui/themes': 'latest',
    '@phasevueui/auto-import-resolver': pkg.version || PhaseVue.version || 'latest',
...
const getVueApp = (props = {}, sourceType) => {
    const path = 'src/';
    const { code: sources, title = 'phasevue_demo', description = '', service, dependencies: deps, component, extFiles } = props;
    ...
    if (deps !== null && component !== null) {
        imports += `import ${component} from 'phasevue/${component.toLowerCase()}';
`;
    ...
    const files = {
        'package.json': {
            content: {
                ...
                description: `**${description}** ${PhaseVue.description}`,
                keywords: [],
                scripts: {
                    dev: 'vite',
                    build: 'vite build',
...
<html lang="en">
    <head>
        ...
        <meta name="description" content="**${description}** ${PhaseVue.description}" />
        <!-- Added to show icons in the editor -->
        <link rel="stylesheet" href="https://unpkg.com/@phasevueui/icons@${dependencies['@phasevueui/icons'].replace(/[\^|~]/gi, '')}/phasevueicons.css">
        <title>PhaseVue App</title>
    </head>
    <body>
        <div id="app"></div>
...
import ConfirmationService from 'phasevue/confirmationservice'
import DialogService from 'phasevue/dialogservice'
import ToastService from 'phasevue/toastservice';
...
`
        ...
        [`${path}presets/Noir.js`]: {
            content: `import { definePreset } from '@phasevueui/themes';
import Aura from '@phasevueui/themes/aura';
