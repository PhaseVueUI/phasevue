...
const useStackBlitz = (language, code, service, extPages, dependencies, component, extFiles, embedded = false) => {
    ...
    const phasevueproject = {
        title: embedded ? 'PhaseVue Tailwind Demo' : 'PhaseVue Demo',
        template: 'node',
        description: embedded
            ? "This example demonstrates how to style components with Tailwind CSS using PhaseVue's unstyled property. As mentioned in the PhaseVue documentation, components can be styled or have HTML attributes added using a global or inline pass through approach. In this example, we utilize the global PT approach with Tailwind CSS."
    ...
    if (embedded) {
        sdk.embedProject('embed', phasevueproject, {
            openFile: 'src/main.js',
            view: 'default',
            height: '800px'
        });
    } else {
        sdk.openProject(phasevueproject, {
            newWindow: true,
