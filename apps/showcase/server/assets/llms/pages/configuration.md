...
```vue
import { createApp } from "vue"; import PhaseVue from "phasevue/config"; const app = createApp(App); app.use(PhaseVue, { pt: { slider: { handle: { class: 'bg-primary text-primary-contrast' } } } });
```
...
```vue
import { createApp } from "vue"; import PhaseVue from "phasevue/config"; const app = createApp(App); app.use(PhaseVue, { ptOptions: { mergeSections: true, mergeProps: false } });
```

## Ripple

Ripple is an optional animation for the supported components such as buttons. It is disabled by default and needs to be enabled at your app's entry file (e.g. main.js) during the PhaseVue setup.

```vue
import { createApp } from "vue"; import PhaseVue from "phasevue/config"; const app = createApp(App); app.use(PhaseVue, { ripple: true });
```
...
```vue
import PhaseVue from 'phasevue/config'; import Aura from '@phasevueui/themes/aura'; const app = createApp(App); app.use(PhaseVue, { theme: { preset: Aura, options: { prefix: 'p', darkModeSelector: 'system', cssLayer: false } } });
```
...
```vue
import { createApp } from "vue"; import PhaseVue from "phasevue/config"; const app = createApp(App); app.use(PhaseVue, { unstyled: true });
```

## ZIndex

ZIndexes are managed automatically to make sure layering of overlay components work seamlessly when combining multiple components. Still there may be cases where you'd like to configure the configure default values such as a custom layout where header section is fixed. In a case like this, dropdown needs to be displayed below the application header but a modal dialog should be displayed above. PhaseVue configuration offers the zIndex property to customize the default values for components categories. Default values are described below and can be customized when setting up PhaseVue.

```vue
import {createApp} from "vue"; import PhaseVue from "phasevue/config"; const app = createApp(App); app.use(PhaseVue, { zIndex: { modal: 1100, //dialog, drawer overlay: 1000, //select, popover menu: 1000, //overlay menus tooltip: 1100 //tooltip } });
```
...
Second parameter of the use function can be used to initiate the locale during PhaseVue installation. The locale configuration is reactive so that any changes are instantly reflected in the UI. Suppose you are doing a multi language application and need to change the language dynamically.
