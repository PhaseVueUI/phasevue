...
Model Context Protocol (MCP) server for PhaseVue component library. Provides AI assistants with comprehensive access to PhaseVue component documentation.

## Claude Code

Add the PhaseVue MCP server using the CLI. After adding, start a new session and use /mcp to verify the connection.
...
```vue
{ "mcpServers": { "phasevue": { "command": "npx", "args": ["-y", "@phasevueui/mcp"] } } }
```
...
```vue
{ "servers": { "phasevue": { "command": "npx", "args": ["-y", "@phasevueui/mcp"] } } }
```

## Windsurf

Edit ~/.codeium/windsurf/mcp_config.json to add the PhaseVue MCP server.
...
```vue
{ "context_servers": { "phasevue": { "command": { "path": "npx", "args": ["-y", "@phasevueui/mcp"] } } } }
```
