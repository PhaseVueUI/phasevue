...
const PhaseVueSymbol = Symbol();

export function usePhaseVue() {
    const PhaseVue = inject(PhaseVueSymbol);

    if (!PhaseVue) {
        throw new Error('PhaseVue is not installed!');
    }
...
export function setup(app, options) {
    ...
    app.config.globalProperties.$phasevue = PhaseVue;
    app.provide(PhaseVueSymbol, PhaseVue);

    clearConfig();
    setupConfig(app, PhaseVue);
...
export function setupConfig(app, PhaseVue) {
    const isThemeChanged = ref(false);

    /*** Methods and Services ***/
    const loadCommonTheme = () => {
        if (PhaseVue.config?.theme === 'none') return;

        // common
        if (!Theme.isStyleNameLoaded('common')) {
            const { primitive, semantic, global, style } = BaseStyle.getCommonTheme?.() || {};
            const styleOptions = { nonce: PhaseVue.config?.csp?.nonce };
    ...
    ThemeService.on('theme:change', function (newTheme) {
        if (!isThemeChanged.value) {
            app.config.globalProperties.$phasevue.config.theme = newTheme;
    ...
    const stopConfigWatcher = watch(
        PhaseVue.config,
        (newValue, oldValue) => {
            PhaseVueService.emit('config:change', { newValue, oldValue });
        },
        { immediate: true, deep: true }
    );

    const stopRippleWatcher = watch(
        () => PhaseVue.config.ripple,
        (newValue, oldValue) => {
            PhaseVueService.emit('config:ripple:change', { newValue, oldValue });
        },
        { immediate: true, deep: true }
    );

    const stopThemeWatcher = watch(
        () => PhaseVue.config.theme,
        (newValue, oldValue) => {
            if (!isThemeChanged.value) {
                Theme.setTheme(newValue);
            }

            if (!PhaseVue.config.unstyled) {
                loadCommonTheme();
            }

            isThemeChanged.value = false;
            PhaseVueService.emit('config:theme:change', { newValue, oldValue });
        },
        { immediate: true, deep: false }
    );

    const stopUnstyledWatcher = watch(
        () => PhaseVue.config.unstyled,
        (newValue, oldValue) => {
            if (!newValue && PhaseVue.config.theme) {
                loadCommonTheme();
            }

            PhaseVueService.emit('config:unstyled:change', { newValue, oldValue });
        },
        { immediate: true, deep: true }
    );
