    ...
    <span ref="container" :id="$id" :class="cx('root')" :style="sx('root')" :data-p="containerDataP" v-bind="ptmi('root')">
        ...
        <slot v-if="showIcon && iconDisplay === 'button' && !inline" name="dropdownbutton" :toggleCallback="onButtonClick">
            <button
                ...
                @click="onButtonClick"
                type="button"
                :aria-label="$phasevue.config.locale.chooseDate"
                aria-haspopup="dialog"
                :aria-expanded="overlayVisible"
                                ...
                                <div :class="cx('header')" v-bind="ptm('header')">
                                    ...
                                    <slot name="prevbutton" :actionCallback="(event) => onPrevButtonClick(event)" :keydownCallback="(event) => onContainerButtonKeydown(event)">
                                        <Button
                                            ...
                                            :disabled="disabled"
                                            :aria-label="currentView === 'year' ? $phasevue.config.locale.prevDecade : currentView === 'month' ? $phasevue.config.locale.prevYear : $phasevue.config.locale.prevMonth"
                                            :unstyled="unstyled"
                                    ...
                                    <div :class="cx('title')" v-bind="ptm('title')">
                                        <template v-if="$phasevue.config.locale.showMonthAfterYear">
                                            <button
                                                ...
                                                :aria-label="$phasevue.config.locale.chooseYear"
                                            ...
                                            <button
                                                ...
                                                :disabled="switchViewButtonDisabled"
                                                :aria-label="$phasevue.config.locale.chooseMonth"
                                    ...
                                    <slot name="nextbutton" :actionCallback="(event) => onNextButtonClick(event)" :keydownCallback="(event) => onContainerButtonKeydown(event)">
                                        <Button
                                            ...
                                            :disabled="disabled"
                                            :aria-label="currentView === 'year' ? $phasevue.config.locale.nextDecade : currentView === 'month' ? $phasevue.config.locale.nextYear : $phasevue.config.locale.nextMonth"
                                            :unstyled="unstyled"
                    ...
                    <div v-if="(showTime || timeOnly) && currentView === 'date'" :class="cx('timePicker')" :data-p="timePickerDataP" v-bind="ptm('timePicker')">
                        <div :class="cx('hourPicker')" v-bind="ptm('hourPicker')" data-pc-group-section="timepickerContainer">
                            <slot name="hourincrementbutton" :callbacks="hourIncrementCallbacks">
                                <Button
                                    ...
                                    :aria-label="$phasevue.config.locale.nextHour"
                                    :unstyled="unstyled"
                            ...
                            <slot name="hourdecrementbutton" :callbacks="hourDecrementCallbacks">
                                <Button
                                    ...
                                    :aria-label="$phasevue.config.locale.prevHour"
                                    :unstyled="unstyled"
                                    @mousedown="onTimePickerElementMouseDown($event, 0, -1)"
                                    @mouseup="onTimePickerElementMouseUp($event)"
                                    @keydown="onContainerButtonKeydown"
                        ...
                        <div :class="cx('minutePicker')" v-bind="ptm('minutePicker')" data-pc-group-section="timepickerContainer">
                            <slot name="minuteincrementbutton" :callbacks="minuteIncrementCallbacks">
                                <Button
                                    :class="cx('pcIncrementButton')"
                                    :aria-label="$phasevue.config.locale.nextMinute"
                                    :disabled="disabled"
                                    :unstyled="unstyled"
                            ...
                            <slot name="minutedecrementbutton" :callbacks="minuteDecrementCallbacks">
                                <Button
                                    :class="cx('pcDecrementButton')"
                                    :aria-label="$phasevue.config.locale.prevMinute"
                                    :disabled="disabled"
                                    :unstyled="unstyled"
                                    @mousedown="onTimePickerElementMouseDown($event, 1, -1)"
                                    @mouseup="onTimePickerElementMouseUp($event)"
                        ...
                        <div v-if="showSeconds" :class="cx('secondPicker')" v-bind="ptm('secondPicker')" data-pc-group-section="timepickerContainer">
                            <slot name="secondincrementbutton" :callbacks="secondIncrementCallbacks">
                                <Button
                                    :class="cx('pcIncrementButton')"
                                    :aria-label="$phasevue.config.locale.nextSecond"
                                    :disabled="disabled"
                                    :unstyled="unstyled"
                            ...
                            <slot name="seconddecrementbutton" :callbacks="secondDecrementCallbacks">
                                <Button
                                    :class="cx('pcDecrementButton')"
                                    :aria-label="$phasevue.config.locale.prevSecond"
                                    :disabled="disabled"
                                    :unstyled="unstyled"
                                    @mousedown="onTimePickerElementMouseDown($event, 2, -1)"
                                    @mouseup="onTimePickerElementMouseUp($event)"
                        ...
                        <div v-if="hourFormat == '12'" :class="cx('ampmPicker')" v-bind="ptm('ampmPicker')">
                            <slot name="ampmincrementbutton" :toggleCallback="(event) => toggleAMPM(event)" :keydownCallback="(event) => onContainerButtonKeydown(event)">
                                <Button
                                    :class="cx('pcIncrementButton')"
                                    :aria-label="$phasevue.config.locale.am"
                                    :disabled="disabled"
                                    :unstyled="unstyled"
                            ...
                            <span v-bind="ptm('ampm')" data-pc-group-section="timepickerlabel">{{ pm ? $phasevue.config.locale.pm : $phasevue.config.locale.am }}</span>
                            <slot name="ampmdecrementbutton" :toggleCallback="(event) => toggleAMPM(event)" :keydownCallback="(event) => onContainerButtonKeydown(event)">
                                <Button
                                    :class="cx('pcDecrementButton')"
                                    :aria-label="$phasevue.config.locale.pm"
                                    :disabled="disabled"
                                    @click="toggleAMPM($event)"
                                    @keydown="onContainerButtonKeydown"
                                    v-bind="timepickerButtonProps"
...
export default {
    ...
    methods: {
        ...
        onOverlayEnter(el) {
            ...
            if (this.autoZIndex) {
                ZIndex.set('overlay', el, this.baseZIndex || this.$phasevue.config.zIndex.overlay);
            }
                    ...
                    if (literal) {
                        ...
                        switch (format.charAt(iFormat)) {
                            case 'd':
                                ...
                                break;
                            case 'D':
                                output += formatName('D', date.getDay(), this.$phasevue.config.locale.dayNamesShort, this.$phasevue.config.locale.dayNames);
                                break;
                            case 'o':
                            ...
                            case 'm':
                                ...
                                break;
                            case 'M':
                                output += formatName('M', date.getMonth(), this.$phasevue.config.locale.monthNamesShort, this.$phasevue.config.locale.monthNames);
                                break;
                            case 'y':
        ...
        formatTime(date) {
            ...
            if (this.hourFormat === '12') {
                output += date.getHours() > 11 ? ` ${this.$phasevue.config.locale.pm}` : ` ${this.$phasevue.config.locale.am}`;
            }
        ...
        parseDateTime(text) {
            let date;
            const amLabel = this.$phasevue.config.locale.am;
            const pmLabel = this.$phasevue.config.locale.pm;
        ...
        populateTime(value, timeString, ampm) {
            ...
            this.pm = ampm.toLowerCase() === this.$phasevue.config.locale.pm.toLowerCase() || ampm.toLowerCase() === 'pm';
                ...
                if (literal) {
                    ...
                    switch (format.charAt(iFormat)) {
                        case 'd':
                            ...
                            break;
                        case 'D':
                            getName('D', this.$phasevue.config.locale.dayNamesShort, this.$phasevue.config.locale.dayNames);
                            break;
                        case 'o':
                            doy = getNumber('o');
                            break;
                        case 'm':
                            ...
                            break;
                        case 'M':
                            month = getName('M', this.$phasevue.config.locale.monthNamesShort, this.$phasevue.config.locale.monthNames);
                            break;
                        case 'y':
                            year = getNumber('y');
                            break;
        ...
        getMonthName(index) {
            return this.$phasevue.config.locale.monthNames[index];
        },
        getYear(month) {
        ...
        createResponsiveStyle() {
            if (this.numberOfMonths > 1 && this.responsiveOptions && !this.isUnstyled) {
                if (!this.responsiveStyleElement) {
                    ...
                    setAttribute(this.responsiveStyleElement, 'nonce', this.$phasevue?.config?.csp?.nonce);
    ...
    computed: {
        ...
        weekDays() {
            let weekDays = [];
            let dayIndex = this.$phasevue.config.locale.firstDayOfWeek;

            for (let i = 0; i < 7; i++) {
                weekDays.push(this.$phasevue.config.locale.dayNamesMin[dayIndex]);
        ...
        sundayIndex() {
            return this.$phasevue.config.locale.firstDayOfWeek > 0 ? 7 - this.$phasevue.config.locale.firstDayOfWeek : 0;
        },
        datePattern() {
            return this.dateFormat || this.$phasevue.config.locale.dateFormat;
        },
        monthPickerValues() {
            ...
            for (let i = 0; i <= 11; i++) {
                monthPickerValues.push({
                    value: this.$phasevue.config.locale.monthNamesShort[i],
        ...
        todayLabel() {
            return this.$phasevue.config.locale.today;
        },
        clearLabel() {
            return this.$phasevue.config.locale.clear;
        },
        weekHeaderLabel() {
            return this.$phasevue.config.locale.weekHeader;
        },
        monthNames() {
            return this.$phasevue.config.locale.monthNames;
        },
