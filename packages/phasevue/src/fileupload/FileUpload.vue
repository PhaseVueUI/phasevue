...
import ProgressBar from 'phasevue/progressbar';
...
export default {
    ...
    methods: {
        ...
        formatSize(bytes) {
            const k = 1024;
            const dm = 3;
            const sizes = this.$phasevue.config.locale?.fileSizeTypes || ['B', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB'];
    ...
    computed: {
        ...
        basicFileChosenLabel() {
            if (this.auto) return this.chooseButtonLabel;
                ...
                return this.$phasevue.config.locale?.fileChosenMessage?.replace('{0}', this.files.length);
            }

            return this.$phasevue.config.locale?.noFileChosenMessage || '';
        },
        hasFiles() {
        ...
        chooseButtonLabel() {
            return this.chooseLabel || this.$phasevue.config.locale.choose;
        },
        uploadButtonLabel() {
            return this.uploadLabel || this.$phasevue.config.locale.upload;
        },
        cancelButtonLabel() {
            return this.cancelLabel || this.$phasevue.config.locale.cancel;
        },
        completedLabel() {
            return this.$phasevue.config.locale.completed;
        },
        pendingLabel() {
            return this.$phasevue.config.locale.pending;
        }
    },
    components: {
        Button,
